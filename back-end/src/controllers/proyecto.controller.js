...
const obtenerReunionesPendientes = async (req, res) => {
  const { id } = req.params;
  try {
    const updateQuery = `UPDATE reunion SET id_estado=3 WHERE id_proyecto=$1 AND fecha<CURRENT_TIMESTAMP AND id_estado!=3;`;
...
const cancelarReunion = async (req, res) => {
  const { id } = req.body;
  try {
    const query = `UPDATE public.reunion SET id_estado = 2 WHERE id = $1`;
    const values = [id];
    await pool.query(query, values);
