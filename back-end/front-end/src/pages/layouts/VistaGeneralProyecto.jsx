...
import { Typography, Box, TextField, Grid, CssBaseline, Tooltip, IconButton, AppBar, Toolbar } from "@mui/material";

import { useSelector } from "react-redux";
import { useSnackbar } from 'notistack';
import { Feed, Source } from "@mui/icons-material";
import VistaGeneralEntrega from "./VistaGeneralEntrega";
import VistaGeneralSolicitud from "./VistaGeneralSolicitud";
import { selectToken } from '../../store/authSlice';
import CustomDataGrid from './DataGrid';
export default function VistaGeneralProyecto() {
    ...
    const abrirVentanaVerSolicitud = (id) => {
        setIdSolicitud(id)
        setOpen(true);
    };

    const cerrarVentanaVerSolicitud = () => {
        setOpen(false);
    }
    ...
    const abrirVentanaVerEntrega = (row, tipo) => {
        setEntrega(row)
        setTipo(tipo)
        setabrirVerEntrega(true);
    };
    const cerrarVentanaVerEntrega = () => {
        setEntrega({})
        setabrirVerEntrega(false);
    }
    ...
    const llenarTablaEntregas = async (endpoint, proyecto_id, setRowsFunc) => {
        try {
    ...
    useEffect(() => {
        infoProyecto()
        llenarTablaEntregas("obtenerEntregasPendientes", id, setEntregasPendientes);
        llenarTablaEntregas("obtenerEntregasCalificadas", id, setEntregasCalificadas);
        llenarTablaEntregas("obtenerEntregasSinCalificar", id, setEntregasPorCalificar);
        llenarTablaSolicitudes("obtenerSolicitudesPendientes", setSolicitudesEnCurso, id);
        llenarTablaSolicitudes("obtenerSolicitudesRechazadas", setSolicitudesRechazadas, id);
        llenarTablaSolicitudes("obtenerSolicitudesAprobadas", setSolicitudesAprobadas, id);
    }, [id]);
                        ...
                        <Box width="100%" m="0 auto" p="5px" display="flex" justifyContent="center">
                            <Box mr="5px">
                                <Tooltip title="Ver Solicitud" >
                                    <IconButton color="secondary" onClick={() => abrirVentanaVerSolicitud(id)}>
                                        <Feed />
                                    </IconButton>
                                </Tooltip>
                            </Box>
    ...
    return (
        <div>
            ...
            {existe ? (
                <Box >
                    <CssBaseline />
                    <Typography variant="h4" color="secondary" sx={{ mb: "5px" }}                    >
                        {proyecto.modalidad || ''}
                    </Typography >
            ...
            <Box mt={4}>
                ...
                <VistaGeneralEntrega
                    open={abrirVerEntrega}
                    onClose={cerrarVentanaVerEntrega}
                    entrega={entrega}
                    tipo={tipo}
                />
            ...
            <Box sx={{ mt: 4 }}>
                ...
                <VistaGeneralSolicitud
                    open={open}
                    onClose={cerrarVentanaVerSolicitud}
                ...
                <CustomDataGrid rows={solicitudesEnCurso} columns={columnsSolPendientes} mensaje="No hay solicitudes pendientes." />

                <Typography variant="h5" color="primary"
                    sx={{ mt: "30px" }}>
                    Aprobadas
                </Typography>
                <CustomDataGrid rows={solicitudesAprobadas} columns={columnsSolAprobadas} mensaje="No hay solicitudes aprobadas." />

                <Typography variant="h5" color="primary"
                    sx={{ mt: "30px" }}>
                    Rechazadas
                </Typography>
                <CustomDataGrid rows={solicitudesRechazadas} columns={columnsSolRechazadas} mensaje="No hay solicitudes rechazadas." />
